...
function renderClickableName(entry: LeaderboardEntry, viewerRole: string, viewerId?: string, className?: string) {
  ...
  if (isStaff) {
    return (
      <Link href={`/admin/users/${entry.id}`} className={cn("hover:text-[#E87154] transition-colors hover:underline cursor-pointer", className)}>
        {entry.name}
      </Link>
    );
  } else if (isSelf) {
        ...
        {top3[1] && (
          <Card className="w-full bg-stone-50 border-stone-200 shadow-md hover:shadow-lg transition-all relative">
             ...
             <CardContent className="pt-6 text-center">
                ...
                <h3 className="font-bold truncate px-2">
                  {renderClickableName(top3[1], viewerRole, viewerId)}
                </h3>
                ...
                {isAllowedToView(top3[1].id) && (
                  <div className="mt-4">
                     <Link href={["ADMIN", "OPERATIONS_MANAGER", "FINANCE"].includes(viewerRole) ? `/admin/users/${top3[1].id}` : `/leaderboard/${top3[1].id}`} className="text-xs font-semibold text-[#E87154] hover:underline">View Profile</Link>
                  </div>
                )}
             </CardContent>
          </Card>
        ...
        {top3[0] && (
          <Card className="w-full border-amber-200 shadow-lg hover:shadow-xl transition-all relative overflow-hidden bg-amber-50">
             ...
             <CardContent className="pt-10 text-center">
                ...
                <h3 className="text-xl font-black truncate px-2 text-stone-900">
                  {renderClickableName(top3[0], viewerRole, viewerId, "text-stone-900")}
                </h3>
                ...
                {isAllowedToView(top3[0].id) && (
                  <div className="mt-6">
                     <Link href={["ADMIN", "OPERATIONS_MANAGER", "FINANCE"].includes(viewerRole) ? `/admin/users/${top3[0].id}` : `/leaderboard/${top3[0].id}`} className="inline-block bg-[#E87154] text-white px-6 py-2 rounded-lg text-sm font-bold hover:bg-[#E87154]/90 transition-colors shadow-sm">View Champion Profile</Link>
                  </div>
                )}
             </CardContent>
          </Card>
        ...
        {top3[2] && (
          <Card className="w-full bg-orange-50/50 border-orange-200 shadow-md hover:shadow-lg transition-all relative">
             ...
             <CardContent className="pt-6 text-center">
                ...
                <h3 className="font-bold truncate px-2">
                  {renderClickableName(top3[2], viewerRole, viewerId)}
                </h3>
                ...
                {isAllowedToView(top3[2].id) && (
                  <div className="mt-4">
                     <Link href={["ADMIN", "OPERATIONS_MANAGER", "FINANCE"].includes(viewerRole) ? `/admin/users/${top3[2].id}` : `/leaderboard/${top3[2].id}`} className="text-xs font-semibold text-[#E87154] hover:underline">View Profile</Link>
                  </div>
                )}
             </CardContent>
          </Card>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="font-bold text-sm text-slate-900">
                          {renderClickableName(entry, viewerRole, viewerId, "text-slate-900")}
                        </span>
