...
import { BookOpen, Filter, Loader2, Calendar as CalendarIcon, User, AlertCircle } from "lucide-react";
...
export default function CustomerFlipbooksPage() {
  ...
  const initialChildId = searchParams.get("childId") || "all";
  ...
  useEffect(() => {
    // If the URL param changes, update the selection
  ...
  async function loadInitialData() {
    try {
      const childData = await getChildProfiles();
      setChildren(childData);
      // We don't need to call loadFlipbooks here since the selectedChildId useEffect will trigger it initially
    } catch (error) {
  ...
  async function loadFlipbooks() {
    try {
      ...
      const childIdParam = selectedChildId === "all" ? undefined : selectedChildId;
      const data = await getCustomerFlipbooks(childIdParam);
  ...
  function handleReadNow(flipbook: any) {
    if (selectedChildId === "all" && children.length > 0) {
      toast.error("Please select a child profile to track reading progress.");
      return;
    }
  ...
  async function handleComplete() {
    ...
    try {
      await updateFlipbookProgress({
        ...
        lastPageRead: 0, 
        completed: true,
        childProfileId: selectedChildId === "all" ? undefined : selectedChildId
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 bg-white dark:bg-slate-900 p-4 rounded-lg border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">Reading As:</span>
        </div>
        <div className="flex-1">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="all">Parent (No child tracking)</SelectItem>
        ...
        {selectedChildId === "all" && children.length > 0 && (
          <div className="flex items-center text-amber-600 text-sm">
            ...
            Select a child to read content and save progress
          </div>
        )}
      </div>
