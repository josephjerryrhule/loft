...
export default function ManualPaymentDetailPage({ params }: PageProps) {
  ...
  const loadDetails = useCallback(async () => {
    ...
    try {
      ...
      if (!data) {
        ...
        router.push("/finance/manual-payments");
        return;
      }
      setPayment(data);
    } catch (err) {
  ...
  if (!payment) {
    return (
      <div className="text-center p-12">
        ...
        <Button onClick={() => router.push("/finance/manual-payments")} className="mt-4 bg-[#E87154] hover:bg-[#D4654A] text-white">
          Back to List
        </Button>
      </div>
    );
